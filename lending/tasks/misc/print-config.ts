...
task("print-config", "Inits the DRE, to have access to all the plugins")
  ...
  .setAction(async ({ pool, dataProvider }, localBRE) => {
    ...
    const addressesProvider = await getLendingPoolAddressesProvider(); // Checks first provider
    ...
    for (const [symbol, address] of Object.entries(
      ...
      const reserveData = await protocolDataProvider.getReserveConfigurationData(
        address
      );
      const tokensAddresses = await protocolDataProvider.getReserveTokensAddresses(
        address
      );
