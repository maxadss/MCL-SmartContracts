...
const testAndExecMintAndTransferTokens = async (
  ...
  const initialMinterBalance = new BigNumber(
    await tokenInstance.balanceOf(minter)
  );
  const initialReceiverBalance = new BigNumber(
    await tokenInstance.balanceOf(receiver)
  );
...
const testAndExecEthTransfer = async (
  ...
  expect(
    new BigNumber(initialReceiverEthBalance).plus(amount).toFixed()
  ).to.be.equal(await web3.eth.getBalance(receiver));
};
...
const testAndExecDistributeToken = async (
  ...
  for (const receiver of receivers) {
    tokenBalancesReceiversBefore.push(
      (await tokenInstance.balanceOf(receiver)).toString()
    );
  }
  const tokenBalancesBefore: iTokenBalances = {
    tokenDistributor: (
      await tokenInstance.balanceOf(tokenDistributorInstance.address)
    ).toString(),
  ...
  const tokenBalanceOfDistributorAfter = (
    await tokenInstance.balanceOf(tokenDistributorInstance.address)
  ).toString();

  expect(parseInt(tokenBalanceOfDistributorAfter)).to.be.within(
    0,
    receivers.length - 1
  );

  for (const [index, receiver] of receivers.entries()) {
    const receiverPercentage = new BigNumber(
      tokenDistribution[1][index]
    ).toFixed();
    const tokenAmountToReceiver = new BigNumber(
      tokenBalancesBefore.tokenDistributor
    )
      .multipliedBy(receiverPercentage)
      .dividedBy(TOKEN_DISTRIBUTOR_PERCENTAGE_BASE)
      .toFixed(0, BigNumber.ROUND_DOWN);
    const tokenBalanceOfReceiverAfter = (
      await tokenInstance.balanceOf(receiver)
    ).toString();
    expect(tokenBalanceOfReceiverAfter).to.be.equal(
      new BigNumber(tokenBalancesBefore.receivers[index])
...
const testAndExecDistributeEth = async (
  ...
  const ethBalancesBefore: iTokenBalances = {
    tokenDistributor: await web3.eth.getBalance(
  ...
  const ethBalanceOfDistributorAfter = await web3.eth.getBalance(
    tokenDistributorInstance.address
  );

  expect(parseInt(ethBalanceOfDistributorAfter)).to.be.within(
    0,
    receivers.length - 1
  );
  ...
  for (const [index, receiver] of receivers.entries()) {
    const receiverPercentage = new BigNumber(
      ethDistribution[1][index]
    ).toFixed();
    const ethAmountToReceiver = new BigNumber(
      ethBalancesBefore.tokenDistributor
    )
      .multipliedBy(receiverPercentage)
      .dividedBy(TOKEN_DISTRIBUTOR_PERCENTAGE_BASE)
      .toFixed(0, BigNumber.ROUND_DOWN);
    ...
    expect(ethBalanceOfReceiverAfter).to.be.equal(
      new BigNumber(ethBalancesBefore.receivers[index])
        .plus(ethAmountToReceiver)
        .toFixed()
    );
  }
};
