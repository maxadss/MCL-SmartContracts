...
contract("mToken: Transfer", async ([deployer, ...users]) => {
  ...
  before("Initializing test variables", async () => {
    ...
    const {
      deployedInstances: { mTokenInstances, tokenInstances },
  ...
  it("User 1 sets the DAI as collateral and borrows, tries to transfer everything back to user 0 (revert expected)", async () => {
    await _lendingPoolInstance.setUserUseReserveAsCollateral(
      _DAI.address,
      true,
      { from: users[1] }
    );
    ...
    await expectRevert(
      _aDAI.transfer(users[0], aDAItoTransfer, { from: users[1] }),
  ...
  it("User 0 tries to transfer 0 balance (revert expected)", async () => {
    await expectRevert(
      _aDAI.transfer(users[1], "0", { from: users[0] }),
  ...
  it("User 1 repays the borrow, transfers aDAI back to user 0", async () => {
    await _lendingPoolInstance.repay(
      ...
      MAX_UINT_AMOUNT,
      users[1],
      { from: users[1], value: oneEther.toFixed(0) }
    );
