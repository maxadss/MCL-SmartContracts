...
export const calcExpectedUserDataAfterDeposit = (
  ...
  expectedUserData.currentmTokenBalance = userDataBeforeAction.currentmTokenBalance.plus(
    amountDeposited
  );

  if (userDataBeforeAction.currentmTokenBalance.eq(0)) {
    expectedUserData.usageAsCollateralEnabled = true;
  } else {
    //if user is redeeming everything, usageAsCollateralEnabled must be false
    if (expectedUserData.currentmTokenBalance.eq(0)) {
  ...
  expectedUserData.principalmTokenBalance = expectedUserData.currentmTokenBalance = calcExpectedmTokenBalance(
    reserveDataBeforeAction,
    userDataBeforeAction,
    txTimestamp
  ...
  expectedUserData.currentmTokenUserIndex = calcExpectedmTokenUserIndex(
    reserveDataBeforeAction,
...
export const calcExpectedUserDataAfterRedeem = (
  ...
  if (amountRedeemed == MAX_UINT_AMOUNT) {
    amountRedeemed = mTokenBalance.toFixed(0);
  }
  expectedUserData.principalmTokenBalance = expectedUserData.currentmTokenBalance = mTokenBalance.minus(
    amountRedeemed
  );
  expectedUserData.currentBorrowBalance = calcExpectedCompoundedBorrowBalance(
    userDataBeforeAction,
  ...
  if (
    expectedUserData.currentmTokenBalance.eq(0) &&
...
export const calcExpectedReserveDataAfterRedeem = (
  ...
  if (amountRedeemed == MAX_UINT_AMOUNT) {
    amountRedeemed = calcExpectedmTokenBalance(
      reserveDataBeforeAction,
      userDataBeforeAction,
      txTimestamp
    ).toFixed();
...
export const calcExpectedUserDataAfterBorrow = (
  ...
  expectedUserData.principalmTokenBalance =
    userDataBeforeAction.principalmTokenBalance;
  expectedUserData.redirectedBalance = userDataBeforeAction.redirectedBalance;
  expectedUserData.interestRedirectionAddress =
    userDataBeforeAction.interestRedirectionAddress;
...
export const calcExpectedUsersDataAfterRedirectInterest = (
  ...
  expectedFromData.principalmTokenBalance = expectedFromData.currentmTokenBalance = calcExpectedmTokenBalance(
    reserveDataBeforeAction,
    fromDataBeforeAction,
    txTimestamp
  );

  expectedToData.principalmTokenBalance = expectedToData.currentmTokenBalance = calcExpectedmTokenBalance(
    reserveDataBeforeAction,
    toDataBeforeAction,
    txTimestamp
  );
  ...
  expectedToData.redirectedBalance = toDataBeforeAction.redirectedBalance.plus(
    expectedFromData.currentmTokenBalance
  );
  ...
  expectedFromData.currentmTokenUserIndex = calcExpectedmTokenUserIndex(
    reserveDataBeforeAction,
    expectedFromData.currentmTokenBalance,
  ...
  expectedToData.currentmTokenUserIndex = calcExpectedmTokenUserIndex(
    reserveDataBeforeAction,
    expectedToData.currentmTokenBalance,
...
const calcExpectedmTokenUserIndex = (
  reserveDataBeforeAction: ReserveData,
...
const calcExpectedmTokenBalance = (
  reserveDataBeforeAction: ReserveData,
  userDataBeforeAction: UserReserveData,
  currentTimestamp: BigNumber
) => {
  ...
  const {
    ...
    currentmTokenUserIndex: userIndexBeforeAction,
    redirectedBalance,
    principalmTokenBalance: principalBalanceBeforeAction,
  } = userDataBeforeAction;
...
const calcExpectedRedirectedBalance = (
  ...
  const balanceIncrease = userDataBeforeAction.currentmTokenBalance.minus(
    userDataBeforeAction.principalmTokenBalance
  );
...
const calculateavailableBorrowsBNB = (
  collateralBalanceETH: BigNumber,
  borrowBalanceETH: BigNumber,
  ...
  let availableBorrowsBNB = collateralBalanceETH.multipliedBy(currentLtv);
  if (availableBorrowsBNB.lt(borrowBalanceETH)) {
    return strToBN("0");
  }
  availableBorrowsBNB = availableBorrowsBNB.minus(borrowBalanceETH);
  const borrowFee = availableBorrowsBNB.multipliedBy(0.0025);
  return availableBorrowsBNB.minus(borrowFee);
};
